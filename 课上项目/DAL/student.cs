using System;
using System.Data;
using System.Text;
using System.Data.SqlClient;
using Maticsoft.DBUtility;//Please add references
namespace WindowsForms_Student.DAL
{
	/// <summary>
	/// 数据访问类:student
	/// </summary>
	public partial class student
	{
		public student()
		{}
		#region  BasicMethod

		/// <summary>
		/// 是否存在该记录
		/// </summary>
		public bool Exists(string SNo)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) from student");
			strSql.Append(" where SNo=@SNo ");
			SqlParameter[] parameters = {
					new SqlParameter("@SNo", SqlDbType.VarChar,30)			};
			parameters[0].Value = SNo;

			return DbHelperSQL.Exists(strSql.ToString(),parameters);
		}


		/// <summary>
		/// 增加一条数据
		/// </summary>
		public bool Add(WindowsForms_Student.Model.student model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into student(");
			strSql.Append("SNo,SName,SSex,DNo,SAge,SBirthday)");
			strSql.Append(" values (");
			strSql.Append("@SNo,@SName,@SSex,@DNo,@SAge,@SBirthday)");
			SqlParameter[] parameters = {
					new SqlParameter("@SNo", SqlDbType.VarChar,30),
					new SqlParameter("@SName", SqlDbType.VarChar,30),
					new SqlParameter("@SSex", SqlDbType.VarChar,10),
					new SqlParameter("@DNo", SqlDbType.VarChar,20),
					new SqlParameter("@SAge", SqlDbType.Int,4),
					new SqlParameter("@SBirthday", SqlDbType.VarChar,50)};
			parameters[0].Value = model.SNo;
			parameters[1].Value = model.SName;
			parameters[2].Value = model.SSex;
			parameters[3].Value = model.DNo;
			parameters[4].Value = model.SAge;
			parameters[5].Value = model.SBirthday;

			int rows=DbHelperSQL.ExecuteSql(strSql.ToString(),parameters);
			if (rows > 0)
			{
				return true;
			}
			else
			{
				return false;
			}
		}
		/// <summary>
		/// 更新一条数据
		/// </summary>
		public bool Update(WindowsForms_Student.Model.student model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update student set ");
			strSql.Append("SName=@SName,");
			strSql.Append("SSex=@SSex,");
			strSql.Append("DNo=@DNo,");
			strSql.Append("SAge=@SAge,");
			strSql.Append("SBirthday=@SBirthday");
			strSql.Append(" where SNo=@SNo ");
			SqlParameter[] parameters = {
					new SqlParameter("@SName", SqlDbType.VarChar,30),
					new SqlParameter("@SSex", SqlDbType.VarChar,10),
					new SqlParameter("@DNo", SqlDbType.VarChar,20),
					new SqlParameter("@SAge", SqlDbType.Int,4),
					new SqlParameter("@SBirthday", SqlDbType.VarChar,50),
					new SqlParameter("@SNo", SqlDbType.VarChar,30)};
			parameters[0].Value = model.SName;
			parameters[1].Value = model.SSex;
			parameters[2].Value = model.DNo;
			parameters[3].Value = model.SAge;
			parameters[4].Value = model.SBirthday;
			parameters[5].Value = model.SNo;

			int rows=DbHelperSQL.ExecuteSql(strSql.ToString(),parameters);
			if (rows > 0)
			{
				return true;
			}
			else
			{
				return false;
			}
		}

		/// <summary>
		/// 删除一条数据
		/// </summary>
		public bool Delete(string SNo)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from student ");
			strSql.Append(" where SNo=@SNo ");
			SqlParameter[] parameters = {
					new SqlParameter("@SNo", SqlDbType.VarChar,30)			};
			parameters[0].Value = SNo;

			int rows=DbHelperSQL.ExecuteSql(strSql.ToString(),parameters);
			if (rows > 0)
			{
				return true;
			}
			else
			{
				return false;
			}
		}
		/// <summary>
		/// 批量删除数据
		/// </summary>
		public bool DeleteList(string SNolist )
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from student ");
			strSql.Append(" where SNo in ("+SNolist + ")  ");
			int rows=DbHelperSQL.ExecuteSql(strSql.ToString());
			if (rows > 0)
			{
				return true;
			}
			else
			{
				return false;
			}
		}


		/// <summary>
		/// 得到一个对象实体
		/// </summary>
		public WindowsForms_Student.Model.student GetModel(string SNo)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 SNo,SName,SSex,DNo,SAge,SBirthday from student ");
			strSql.Append(" where SNo=@SNo ");
			SqlParameter[] parameters = {
					new SqlParameter("@SNo", SqlDbType.VarChar,30)			};
			parameters[0].Value = SNo;

			WindowsForms_Student.Model.student model=new WindowsForms_Student.Model.student();
			DataSet ds=DbHelperSQL.Query(strSql.ToString(),parameters);
			if(ds.Tables[0].Rows.Count>0)
			{
				return DataRowToModel(ds.Tables[0].Rows[0]);
			}
			else
			{
				return null;
			}
		}


		/// <summary>
		/// 得到一个对象实体
		/// </summary>
		public WindowsForms_Student.Model.student DataRowToModel(DataRow row)
		{
			WindowsForms_Student.Model.student model=new WindowsForms_Student.Model.student();
			if (row != null)
			{
				if(row["SNo"]!=null)
				{
					model.SNo=row["SNo"].ToString();
				}
				if(row["SName"]!=null)
				{
					model.SName=row["SName"].ToString();
				}
				if(row["SSex"]!=null)
				{
					model.SSex=row["SSex"].ToString();
				}
				if(row["DNo"]!=null)
				{
					model.DNo=row["DNo"].ToString();
				}
				if(row["SAge"]!=null && row["SAge"].ToString()!="")
				{
					model.SAge=int.Parse(row["SAge"].ToString());
				}
				if(row["SBirthday"]!=null)
				{
					model.SBirthday=row["SBirthday"].ToString();
				}
			}
			return model;
		}

		/// <summary>
		/// 获得数据列表
		/// </summary>
		public DataSet GetList(string strWhere)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select SNo,SName,SSex,DNo,SAge,SBirthday ");
			strSql.Append(" FROM student ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
			return DbHelperSQL.Query(strSql.ToString());
		}

		/// <summary>
		/// 获得前几行数据
		/// </summary>
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ");
			if(Top>0)
			{
				strSql.Append(" top "+Top.ToString());
			}
			strSql.Append(" SNo,SName,SSex,DNo,SAge,SBirthday ");
			strSql.Append(" FROM student ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
			strSql.Append(" order by " + filedOrder);
			return DbHelperSQL.Query(strSql.ToString());
		}

		/// <summary>
		/// 获取记录总数
		/// </summary>
		public int GetRecordCount(string strWhere)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) FROM student ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
			object obj = DbHelperSQL.GetSingle(strSql.ToString());
			if (obj == null)
			{
				return 0;
			}
			else
			{
				return Convert.ToInt32(obj);
			}
		}
		/// <summary>
		/// 分页获取数据列表
		/// </summary>
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("SELECT * FROM ( ");
			strSql.Append(" SELECT ROW_NUMBER() OVER (");
			if (!string.IsNullOrEmpty(orderby.Trim()))
			{
				strSql.Append("order by T." + orderby );
			}
			else
			{
				strSql.Append("order by T.SNo desc");
			}
			strSql.Append(")AS Row, T.*  from student T ");
			if (!string.IsNullOrEmpty(strWhere.Trim()))
			{
				strSql.Append(" WHERE " + strWhere);
			}
			strSql.Append(" ) TT");
			strSql.AppendFormat(" WHERE TT.Row between {0} and {1}", startIndex, endIndex);
			return DbHelperSQL.Query(strSql.ToString());
		}

		/*
		/// <summary>
		/// 分页获取数据列表
		/// </summary>
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			SqlParameter[] parameters = {
					new SqlParameter("@tblName", SqlDbType.VarChar, 255),
					new SqlParameter("@fldName", SqlDbType.VarChar, 255),
					new SqlParameter("@PageSize", SqlDbType.Int),
					new SqlParameter("@PageIndex", SqlDbType.Int),
					new SqlParameter("@IsReCount", SqlDbType.Bit),
					new SqlParameter("@OrderType", SqlDbType.Bit),
					new SqlParameter("@strWhere", SqlDbType.VarChar,1000),
					};
			parameters[0].Value = "student";
			parameters[1].Value = "SNo";
			parameters[2].Value = PageSize;
			parameters[3].Value = PageIndex;
			parameters[4].Value = 0;
			parameters[5].Value = 0;
			parameters[6].Value = strWhere;	
			return DbHelperSQL.RunProcedure("UP_GetRecordByPage",parameters,"ds");
		}*/

		#endregion  BasicMethod
		#region  ExtensionMethod

		#endregion  ExtensionMethod
	}
}

